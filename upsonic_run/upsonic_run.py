...
class Upsonic_Run:
    ...
    def add_task(
        self, name, endless=False, thread=False, args_for_func=(), kwargs_for_func={}
    ):
        try_number = 0
        while try_number <= self.try_number or self.try_number == 0:
            ...
            runners = [
                runner
                for runner in all_records
                if runner.startswith("RUNNER-")
                and (time.time() - all_records[runner]) <= self.interval * 2
            ]
            old_runners = [
                runner
                for runner in all_records
    ...
    def run(self, name, args_for_func=(), kwargs_for_func={}):
        self.add_task(
            name, args_for_func=args_for_func, kwargs_for_func=kwargs_for_func
        )
